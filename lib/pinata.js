...
export async function fetchFromPinata(cid, config) {
  try {
    ...
    // In production, try multiple IPFS gateways to avoid CORS issues
    if (isProduction) {
      console.log("🌐 Production environment detected - trying multiple IPFS gateways");
      ...
      for (let i = 0; i < gateways.length; i++) {
        ...
        try {
          ...
          if (response.ok) {
            console.log(`✅ Success with gateway ${i + 1}:`, gatewayUrl);
            ...
            console.log(`❌ Gateway ${i + 1} failed:`, response.status, response.statusText);
          }
        } catch (error) {
          console.log(`❌ Gateway ${i + 1} error:`, error.message);
          // Continue to next gateway
        }
      }
      
      throw new Error("All IPFS gateways failed in production environment");
    }

    // For development/localhost, try private gateway first
    let gatewayUrl;
    ...
    // Handle CORS and other network errors by trying multiple public gateways
    if (
      ...
      console.log(
        "🌐 CORS or network error - trying multiple IPFS gateways as fallback"
      );
      ...
      for (let i = 0; i < fallbackGateways.length; i++) {
        const gatewayUrl = fallbackGateways[i];
        console.log(`🔗 Fallback gateway ${i + 1}/${fallbackGateways.length}:`, gatewayUrl);
        ...
        try {
          ...
          if (response.ok) {
            console.log(`✅ Fallback success with gateway ${i + 1}:`, gatewayUrl);
            const contentType = response.headers.get("content-type");
            if (contentType?.includes("application/json")) {
              return await response.json();
            } else {
            ...
            console.log(`❌ Fallback gateway ${i + 1} failed:`, response.status, response.statusText);
          }
        } catch (fallbackError) {
          console.log(`❌ Fallback gateway ${i + 1} error:`, fallbackError.message);
          // Continue to next gateway
        }
      }
      
      console.error("All fallback gateways also failed");
    }

...
export async function fetchFromPinata(cid, config) {
  try {
    ...
    // In production, skip private gateway and go directly to public gateway
    // to avoid CORS issues with private gateways
    if (isProduction) {
      console.log("🌐 Production environment detected - using public gateway directly");
      const publicUrl = `https://gateway.pinata.cloud/ipfs/${cid}`;
      console.log("🔗 Fetching from public gateway:", publicUrl);
      ...
      if (publicResponse.ok) {
        ...
        throw new Error(
          `Failed to fetch from public gateway: ${publicResponse.status} ${publicResponse.statusText}`
        );
      }
    }

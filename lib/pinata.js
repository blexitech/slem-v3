...
export async function fetchFromPinata(cid, config) {
  try {
    ...
    const isProduction =
      ...
      (window.location.hostname.includes("amplifyapp.com") || window.location.hostname.includes("studentloanexp.io"));

    // In production, use ipfs.io directly for best performance and reliability
    if (isProduction) {
      console.log(
        "🌐 Production environment detected - using ipfs.io gateway directly"
      );
      const ipfsUrl = `https://ipfs.io/ipfs/${cid}`;
      console.log("🔗 Fetching from ipfs.io:", ipfsUrl);
      ...
      if (response.ok) {
        console.log("✅ Success with ipfs.io gateway");
        ...
        throw new Error(
          `Failed to fetch from ipfs.io: ${response.status} ${response.statusText}`
        );
      }
    }
    ...
    // Handle CORS and other network errors by trying ipfs.io as fallback
    if (
      ...
      console.log("🌐 CORS or network error - trying ipfs.io as fallback");

      try {
        const fallbackUrl = `https://ipfs.io/ipfs/${cid}`;
        console.log("🔗 Fallback to ipfs.io:", fallbackUrl);
        ...
        if (response.ok) {
          console.log("✅ Fallback success with ipfs.io");
          const contentType = response.headers.get("content-type");
          if (contentType?.includes("application/json")) {
            return await response.json();
          } else {
          ...
          console.log(
            "❌ ipfs.io fallback failed:",
            response.status,
            response.statusText
          );
        }
      } catch (fallbackError) {
        console.error("ipfs.io fallback also failed:", fallbackError);
      }
    }
